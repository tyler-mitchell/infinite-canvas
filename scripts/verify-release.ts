...
const directory = await mkdtemp(join(tmpdir(), "infinite-canvas-release-"));

try {
  await writeFile(
    join(directory, "package.json"),
    `${JSON.stringify({ name: "infinite-canvas-verifier", private: true, type: "module" })}\n`,
  );
  ...
  if (release.trim() !== "false") throw new Error(`${spec} has no GitHub Release.`);
