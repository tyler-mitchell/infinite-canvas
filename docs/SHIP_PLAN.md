...
## Release is Bumpy now, not a hand-managed version field — 2026-08-12

Versioning, changelog, and publication run through Bumpy. A bump file reaching `main` opens or
updates `bumpy/version-packages`; merging that pull request publishes through npm OIDC.
...
CI runs the package build, `publint`, `attw`, and the clean consumer install before the version
pull request can merge. The one-time initial publication creates the npm name and configures
`release.yml` as its trusted publisher; every consumer release after that is tokenless CI.
