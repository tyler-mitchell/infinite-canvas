...
- **Keyboard commands** — `Escape`, `Mod+A`, `Shift+1` (fit all), `Shift+2` (fit selection), arrow-key nudge, `Alt+Arrow` (focus the neighbouring window — group members first, and a floating window over a group searches that group first too), `Mod+0` (reset zoom), `Alt+Shift+Arrow` (resize the active window), `Mod+Shift+Arrow` (place it in a half of the visible canvas), `Mod+Shift+Enter` (fill it); replaceable through `hotkeyBindings`. The vocabulary reads: bare arrow moves a little, `Shift` moves a lot, `Alt` moves focus, `Alt+Shift` changes the shape, `Mod+Shift` tiles. Centring and the quarter-tiles are commands with no default chord, because the canvas `preventDefault()`s every chord it owns and the obvious candidates are browser devtools shortcuts. A chord the canvas owns is swallowed even when its command is unavailable, so a focus move at the edge of your windows can never fall through to the browser's Back. A group's tab strip is a single tab stop with `Arrow`/`Home`/`End` moving between tabs and `Enter`/`Space` activating, rather than one tab stop per tab.
