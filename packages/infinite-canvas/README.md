...
- **Keyboard commands** — `Escape`, `Mod+A`, `Shift+1` (fit all), `Shift+2` (fit selection), arrow-key nudge, `Alt+Arrow` (focus the neighbouring window — group members first, and a floating window over a group searches that group first too), `Mod+0` (reset zoom), `Mod+Alt+Arrow` (place the active window in a half of the visible canvas), `Mod+Alt+Enter` (fill it), `Mod+Alt+C` (centre it); replaceable through `hotkeyBindings`. A chord the canvas owns is swallowed even when its command is unavailable, so a focus move at the edge of your windows can never fall through to the browser's Back. A group's tab strip is a single tab stop with `Arrow`/`Home`/`End` moving between tabs and `Enter`/`Space` activating, rather than one tab stop per tab.
